...
class ExamQuestionLine(models.Model):
    ...
    correct_answer = fields.Char(string='Correct Answer', help="Specify the correct option (A, B, C, or D).")
    ...
    # Essay
    essay_answer = fields.Text(string='Essay Answer', help="Provide the essay response here.")
    ...
    @api.constrains('question_type', 'option_a', 'option_b', 'option_c', 'option_d', 'true_false_answer',
                    'essay_answer', 'match_question', 'match_answer')
        ...
        for record in self:
            if record.question_type == 'multiple_choice':
                if not any([record.option_a, record.option_b, record.option_c, record.option_d]):
                    raise ValidationError(_("At least one option must be provided for multiple-choice questions."))
